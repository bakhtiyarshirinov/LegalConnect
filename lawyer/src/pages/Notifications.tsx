...
// 4 categories by keywords in type+title (types like "appointment" cover several events).
function getIcon(type: string, title: string) {
  const s = `${type ?? ''} ${title ?? ''}`.toLowerCase()
  if (/cancel|ləğv|unverified|reject|rədd|expired|overdue|müddət/.test(s)) return <AlertTriangle size={16} color="#dc2626" />
  if (/verified|confirm|approved|complet|təsdiq/.test(s)) return <CheckCircle2 size={16} color="#16a34a" />
  if (/appointment|booking|görüş|schedule|cədvəl/.test(s)) return <Calendar size={16} color="#2563eb" />
  if (/message|chat|mesaj/.test(s)) return <MessageSquare size={16} color="#16a34a" />
...
export default function Notifications() {
  ...
  const invalidate = () => {
    ...
    qc.invalidateQueries({ queryKey: ['notif-unread-count', user.userId] })
  }
  ...
  return (
    <motion.div initial="hidden" animate="visible" variants={pageVariant} style={{ padding: '32px 36px', background: '#FAFAFA', minHeight: '100%' }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        {([['all', `Hamısı (${notifications.length})`], ['unread', `Oxunmamış (${unreadCount})`]] as const).map(([key, label]) => (
          <button
            key={key}
            onClick={() => setFilter(key)}
            style={{
              padding: '6px 14px', borderRadius: 999, fontSize: 13, fontWeight: 600, cursor: 'pointer',
              border: '1px solid ' + (filter === key ? '#0A0A0A' : '#E8E8E8'),
              background: filter === key ? '#0A0A0A' : '#FFFFFF',
              color: filter === key ? '#FFFFFF' : '#6B6B6B',
            }}
          >
            {label}
          </button>
          ...
          <motion.div initial="hidden" animate="visible" variants={stagger}>
            {shown.map((n, index) => (
              <motion.div
                ...
                style={{
                  padding: '18px 24px',
                  borderBottom: index < shown.length - 1 ? '1px solid #E8E8E8' : 'none',
