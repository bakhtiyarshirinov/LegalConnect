...
import { getChats, getMessages, sendMessage, type Chat, type Message } from '../api/chats'
...
export default function Chat() {
  ...
  const onSignalRMessage = useCallback(
    (chatId: string, senderId: string, content: string, sentAt: string) => {
      if (senderId === user?.userId) return // skip own echo
  ...
  const { joinChat, leaveChat } = useSignalR(onSignalRMessage)
  ...
  const handleSend = async () => {
    ...
    try {
      await sendMessage(selectedChat.id, user.userId, content)
      qc.invalidateQueries({ queryKey: ['messages', selectedChat.id] })
      qc.invalidateQueries({ queryKey: ['chats'] })
    } catch {
        ...
        {selectedChat ? (
          <>
            ...
            >
              ...
              >
                <Send size={16} color={input.trim() ? '#FFFFFF' : '#6B6B6B'} />
              </motion.button>
            </div>
          </>
        ) : (
