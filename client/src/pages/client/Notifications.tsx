...
function getIcon(type: string) {
  if (type?.toLowerCase().includes('appointment') || type?.toLowerCase().includes('booking')) {
    return <Calendar size={16} color="#2563eb" />
  }
  if (type?.toLowerCase().includes('message') || type?.toLowerCase().includes('chat')) {
    return <MessageSquare size={16} color="#16a34a" />
  }
      ...
      <div style={{
        ...
        {isLoading ? (
          ...
          <div style={{ textAlign: 'center', padding: '64px 24px', color: '#A3A3A3' }}>
            ...
            <p style={{ fontSize: 14 }}>You're all caught up! Notifications about appointments and messages will appear here.</p>
          </div>
        ) : (
          <motion.div initial="hidden" animate="visible" variants={stagger}>
            {notifications.map((n, index) => (
              <motion.div
                ...
                onMouseEnter={(e) => {
                  if (!n.isRead) (e.currentTarget as HTMLElement).style.background = '#EFEFEF'
                }}
                onMouseLeave={(e) => {
                  if (!n.isRead) (e.currentTarget as HTMLElement).style.background = '#F5F5F5'
                }}
              >
                {/* Icon */}
                <div style={{
                  width: 36, height: 36, borderRadius: 10, flexShrink: 0,
                  background: n.isRead ? '#F5F5F5' : '#FFFFFF',
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8, marginBottom: 4 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                      {!n.isRead && (
                        <div style={{
                          width: 8, height: 8, borderRadius: '50%', background: '#0A0A0A', flexShrink: 0,
                        }} />
                      )}
                      <span style={{ fontSize: 12, color: '#A3A3A3', whiteSpace: 'nowrap' }}>
