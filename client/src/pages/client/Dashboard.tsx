...
export default function ClientDashboard() {
  ...
  const stats = [
    ...
    { label: 'Upcoming', value: appointments.filter((a) => a.status === 'Pending' || a.status === 'Confirmed').length },
  ...
  return (
    <motion.div initial="hidden" animate="visible" variants={pageVariant} style={{ padding: '32px 36px', background: '#F5F5F5', minHeight: 'calc(100vh - 64px)' }}>
      <div style={{ marginBottom: 32 }}>
        <h1 style={{ fontSize: 26, fontWeight: 800, color: '#0A0A0A', letterSpacing: '-0.03em', marginBottom: 6 }}>Welcome back, {user.fullName.split(' ')[0]}! 👋</h1>
      ...
      <motion.div initial="hidden" animate="visible" variants={stagger} style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 32 }}>
        {stats.map((s) => (
          <motion.div key={s.label} variants={item} whileHover={{ y: -2, boxShadow: '0 4px 16px rgba(0,0,0,0.08)' }} transition={{ type: 'spring', stiffness: 400, damping: 25 }}
            style={{ background: '#FFFFFF', border: '1px solid #F0F0F0', borderTop: '3px solid #0A0A0A', borderRadius: 16, padding: 24, boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
        <div style={{ background: '#FFFFFF', border: '1px solid #F0F0F0', borderRadius: 16, padding: 24, boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
            <h2 style={{ fontSize: 16, fontWeight: 700, color: '#0A0A0A', display: 'flex', alignItems: 'center', gap: 8 }}><Calendar size={16} /> Upcoming</h2>
            <Link to="/lawyers" style={{ fontSize: 13, color: '#6B6B6B', display: 'flex', alignItems: 'center', gap: 4, textDecoration: 'none' }}>Book new <ArrowRight size={12} /></Link>
          </div>
          {loadingAppts ? (
            ...
            <div style={{ textAlign: 'center', padding: '32px 0', color: '#A3A3A3' }}>
              ...
              <Link to="/lawyers"><Button size="sm" style={{ marginTop: 12 }}>Find a Lawyer</Button></Link>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {upcoming.map((a) => (
                <div key={a.id} style={{ padding: 16, background: '#F5F5F5', borderRadius: 12, border: '1px solid #F0F0F0' }}>
          ...
          {loadingNotifs ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {notifications.slice(0, 5).map((n) => (
                <div key={n.id} style={{ padding: '12px 14px', borderRadius: 10, background: n.isRead ? '#FFFFFF' : '#F5F5F5', border: '1px solid #E8E8E8', borderLeft: `3px solid ${n.isRead ? '#E8E8E8' : '#0A0A0A'}` }}>
