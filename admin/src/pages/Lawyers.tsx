...
import { CheckCircle, MapPin, Briefcase, DollarSign, Hash, Star } from 'lucide-react'
  ...
  return (
    <div>
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        ...
        <p className="text-[#6B6B6B] text-sm mt-1">Gözləyən vəkil qeydiyyatlarını yoxlayın və təsdiqləyin</p>
      </motion.div>
      ...
      {!isLoading && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-6">
          <span className="inline-flex items-center gap-2 px-3 py-1.5 bg-[#F5F5F5] border border-[#E8E8E8] rounded-xl text-sm font-medium text-[#6B6B6B]">
            ...
            {lawyers.length} gözləyən {lawyers.length === 1 ? 'vəkil' : 'vəkil'}
          </span>
        </motion.div>
      )}
